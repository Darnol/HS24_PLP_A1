...
// By using arrays and mutable references, the whole operation should be relatively memory efficient I think

fn main() {

    let mut array_to_sort_int: [isize; 5] = [5,4,3,2,1];
    println!("Sort int array {:?}", array_to_sort_int);
    let right = (array_to_sort_int.len() as isize) - 1;
    ...
    let right = (array_to_sort_float.len() as isize) - 1;
    ...
    let right = (array_to_sort_str.len() as isize) - 1;
    ...
    let right = (array_to_sort_tuple.len() as isize) - 1;
    ...
    let right: isize = (array_empty.len() as isize) - 1;
    ...
    let right: isize = (array_len_1.len() as isize) - 1;
    quicksort(&mut array_len_1, 0, right);
    ...
    let right: isize = (array_len_2_identical.len() as isize) - 1;
    quicksort(&mut array_len_2_identical, 0, right);
...
fn quicksort<T: PartialOrd>(array: &mut [T], left: isize, right: isize) {
    ...
    if pivot_index > 0 {
        quicksort(array, left as isize, (pivot_index-1) as isize);
    }
    // partition the right half
    quicksort(array, (pivot_index+1) as isize, right as isize);
